...
- [x] Step 3.1: Write failing test for the scanner (RED)
- [x] Step 3.2: Implement with fast-glob (GREEN)
- [x] Step 3.3: Filter by extension, respect .gitignore patterns

## Module 4: AST-Based Detection Engine
Goal: Parse a file and return all unhandled fetch/axios call locations.
- [x] Step 4.1: Write failing tests for the detector with code fixtures (RED)
- [x] Step 4.2: Parse source to AST with typescript-estree (GREEN)
- [x] Step 4.3: Walk the AST — find AwaitExpression → CallExpression → fetch/axios
- [x] Step 4.4: Check if the AwaitExpression is inside a TryStatement ancestor
- [x] Step 4.5: Return structured findings with file, line, column, code snippet

## Module 5: CLI Output & Polish
Goal: The CLI feels like a real tool — structured JSON, human output, exit codes.
- [x] Step 5.1: Write failing tests for JSON output shape (RED)
- [x] Step 5.2: Implement --json flag that emits machine-readable output (GREEN)
- [x] Step 5.3: Human-readable output with file:line — message format
- [x] Step 5.4: Exit code 0 (clean) / 1 (violations found)
- [x] Step 5.5: Verify --help, --json, and directory argument end-to-end

## Module 6: ESLint Rule (TDD with RuleTester)
Goal: The same detection logic as a real ESLint rule that works in editors and CI.
- [x] Step 6.1: Understand RuleTester — how it differs from Jest
- [x] Step 6.2: Set up eslint-plugin project structure
- [x] Step 6.3: Write failing RuleTester tests — valid and invalid cases (RED)
- [x] Step 6.4: Implement the rule's AST visitor (GREEN)
- [ ] Step 6.5: Handle edge cases — .catch(), Promise.all, optional chaining
- [x] Step 6.6: Add rule metadata (type, docs, schema)
